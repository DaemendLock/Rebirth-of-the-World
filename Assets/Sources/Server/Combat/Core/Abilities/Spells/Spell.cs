...
namespace Core.Combat.Abilities
{
    public abstract class Spell : Castable
    {
        ...
        public abstract bool CanCast(Unit caster, Unit target, SpellValueProvider values);
        ...
        public ActionRecord ApplyEffect(int index, float modification, Unit caster, Unit target) => _data.Effects[index].ApplyEffect(new SpellEffects.EffectApplicationData(caster, target, Id), modification);
        ...
        protected static bool CanTarget(Unit caster, Unit target, TargetTeam team) =>
            team switch
            {
                TargetTeam.None => target == null,
                TargetTeam.Ally => caster.CanHelp(target),
                TargetTeam.Enemy => caster.CanHurt(target),
                TargetTeam.Both => target != null,
                _ => throw new System.ArgumentException(nameof(team))
            };

        protected static bool CanReach(Unit caster, Unit target, float range) => caster == target || (caster.Position - target.Position).sqrMagnitude <= range * range;
